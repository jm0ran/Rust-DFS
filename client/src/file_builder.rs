use std::{collections::{HashMap, HashSet}, io::{Read, Write}, net::{Shutdown, TcpStream}, thread::JoinHandle};
...
impl FileBuilder {
     ...
     */
    pub fn new(request_file_path: String, output_file_path: String, size: u64, file_hash: String) -> FileBuilder {
        let total_blocks = (size / config::BLOCK_SIZE) + if size % config::BLOCK_SIZE != 0 { 1 } else { 0 };
    ...
    pub fn add_distributors(&mut self, distributors: HashSet<String>){
        for distributor in &distributors{
            if self.distributors_in_use.contains(distributor) || self.distributors_available.contains(distributor) {
                continue;
            }else{
    ...
    pub fn start_next_block(&mut self){
        if !(self.distributors_available.len() > 0) { // If there are no distributors available do not start the next block
            return;
        }
        for i in 0..self.block_states.len() {
            if self.block_states[i] == BlockState::Waiting {
                ...
                // Get a distributor
                let distributor = self.distributors_available.iter().next().cloned().unwrap(); //Already verified there will be a distributor, if it crashes then my thread safety is lacking
    ...
    pub fn spawn_download_thread(&mut self, block_num: u64, distributor: String) {
        ...
        self.active_threads.insert(block_num, std::thread::spawn(move || {
    ...
    pub fn create_block_request(&self, block_num: u64) -> Vec<String> {
        ...
        request.push(format!("#S RDFS 0.1 BLOCK_REQUEST {}\n", config::BLOCK_SIZE));
